...
# a = enumerate(zip(train_loader_a, train_loader_b))
# for b in a:
#     print(len(b))
while True:
    for it, ([images_a, label_a], [images_b, label_b]) in enumerate(zip(train_loader_a, train_loader_b)):
        trainer.update_learning_rate()
        images_a, images_b = images_a.cuda().detach(), images_b.cuda().detach()
        label_a, label_b = to_onehot(label_a, config['num_classes']), to_onehot(label_b, config['num_classes'])
        
        
        # print("Iteration ", it)
        with Timer("Elapsed time in update: %f"):
            # Main training code
            trainer.dis_update(images_a, images_b, label_a, label_b, config)
            trainer.gen_update(images_a, images_b, label_a, label_b, config)
            if (it+1) % accum_iter == 0:
                trainer.gen_opt.step()
                trainer.dis_opt.step()
                print("Gradient accumulated!")
