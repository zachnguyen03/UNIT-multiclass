...
class MUNIT_Trainer(nn.Module):
    ...
    def recon_criterion(self, output, target):
        return torch.mean(torch.abs(output - target))

    def style_encoder_criterion(self, output, target):
    ...
    def forward(self, x_a, x_b):
        ...
        c_a, s_a_fake, _ = self.gen_a.encode(x_a)
        c_b, s_b_fake, _ = self.gen_b.encode(x_b)
    ...
    def gen_update(self, x_a, x_b, y_a, y_b, hyperparameters):
        ...
        # encode
        c_a, s_a_prime, y_hat_a = self.gen_a.encode(x_a)
        c_b, s_b_prime, y_hat_b = self.gen_b.encode(x_b)
        ...
        c_a_recon, s_b_recon, _ = self.gen_b.encode(x_ab)
        ...
        # style encoder classifier loss
        # print(y_hat_a)
        # print(y_a)
        self.loss_cls_style = self.style_encoder_criterion(y_hat_a, y_a) + \
                                self.style_encoder_criterion(y_hat_b, y_b)
        # GAN loss
        ...
        self.loss_gen_total = hyperparameters['gan_w'] * self.loss_gen_adv_a + \
                              ...
                              hyperparameters['bce_w'] * self.loss_cls_style
        print(
            "L_total: %.4f, L_gan: %.4f,  Lx: %.4f, Lrecycle:%.4f, Lc: %.4f, Ls: %.4f, L_bce: %.4f" % (
                self.loss_gen_total,
                ...
                hyperparameters['recon_c_w'] * (self.loss_gen_recon_c_a + self.loss_gen_recon_c_b),
    ...
    def dis_update(self, x_a, x_b, y_a, y_b, hyperparameters):
        ...
        x_ab = self.gen_b.decode(c_a, s_b)
        # D loss
        self.loss_dis_a = self.dis_a.calc_dis_loss(x_ba.detach(), x_a, y_a)
        self.loss_dis_b = self.dis_b.calc_dis_loss(x_ab.detach(), x_b, y_b)
